...
import {dynamicSort, loggerMobx} from '../../helpers';
        ...
        switch (action) {
            ...
            case 'filterByClub': {
                ...
                if (!query.length && position === 'ALL') {
                    return data
                        .filter((player) => parseInt(player.price) >= parseInt(min) && parseInt(player.price) <= parseInt(max))
            ...
            case 'filterByPosition': {
                ...
                return data.filter(
                    (player) => player.price >= parseInt(min) && player.price <= parseInt(max),
                ).filter((player) => player.club.trim() === club.trim()).filter((player) => {
                    const {firstName, lastName} = player;
                    const fullName = `${firstName} ${lastName}`;
                    return fullName.trim().toLocaleLowerCase().includes(query.trim().toLowerCase());
                });
